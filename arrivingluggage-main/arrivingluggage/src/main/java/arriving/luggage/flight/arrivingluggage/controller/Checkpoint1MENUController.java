...
public class Checkpoint1MENUController
{
	...
	public String getCheckpoint1(Model model)
	{
		...
		ResponseEntity<Checkpoint1[]> checkpoint1Response = restTemplate.getForEntity(uri, Checkpoint1[].class);
		
		// parse a JSON data to array of object
		Checkpoint1 checkpoint1s[] = checkpoint1Response.getBody();
	...
	public String getCheckpoint1 (@PathVariable Integer Checkpoint1Id, Model model)
	{
		...
		ResponseEntity<Truck[]> responseTruck =
				restTemplateTruck.getForEntity("http://localhost:8080/arriving/api/trucks", Truck[].class);
		
		Truck truckArray[] = responseTruck.getBody();
		
		// Parse array to a list
		...
		ResponseEntity<Luggage[]> responseLuggage =
				restTemplateLuggage.getForEntity("http://localhost:8080/arriving/api/luggages", Luggage[].class);
